...
- **貼上正確歌詞只做時間對齊**：完整保留你提供的原文，沿用既有字幕時間軸，**不會再呼叫 Whisper 或改寫歌詞**；要微調可使用字幕工作台逐句打點
- **字幕工作台**：可直接修改歌詞、匯入 TXT / LRC / SRT / VTT，播放歌曲時逐句按鍵打點對齊；儲存後立即生效，不需要重新跑 AI
...
- **兩種錄製方式：**
  ...
  - **Cover 影片**：鏡頭 + 上字幕 → `cover.mp4`（H.264 + AAC）
    - 鏡像預覽（像照鏡子，所見即所得）；字型、字體大小可選，**字幕位置可拖曳**（每首歌記憶）；可選擇是否把字幕燒錄到影片
...
4. 如果歌詞不準，開啟**字幕工作台**：直接修改句子、匯入 TXT / LRC / SRT / VTT，或貼上純文字歌詞。純文字歌詞可播放歌曲後，用「標記開始」與「標記結束並下一句」逐句對齊，再儲存。
5. 錄製：**純音訊**（麥克風 + 伴奏 → MP3）或 **Cover 影片**（鏡頭 + 字幕 → MP4）。
   兩者不能同時錄。
